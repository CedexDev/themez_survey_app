...
<div class="reg-cont">
    <div class="reg-mid">
        ...
        <div class="reg-head">
            ...
            <p>{{ __('Welcome back, sign in to access your account') }}
            </p>
        </div>
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="form-group text-center">
                <button type="submit" class="button-submit">{{ __('Sign In') }}</button>
            </div>
        </form>
        <p class="text-center">
            <small>
                <a href="{{ route('password.request') }}"
                    class="text-center text-white">{{ __('Reset password') }}</a></small>
        </p>
        <p class="text-center">
            <small>
                <span class="text-white-60">Not a member?</span> <a href="{{ route('register') }}"
                    class="text-center text-white">{{ __("Create an account") }}</a></small>
        </p>
    </div>
</div>
